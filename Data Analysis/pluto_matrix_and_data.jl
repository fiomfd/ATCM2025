...
imgRGB = RGB.(RRR, GGG, BBB)

ZZZ = zeros(n1, n1)
imgR = RGB.(RRR, ZZZ, ZZZ)   # 赤だけ
imgG = RGB.(ZZZ, GGG, ZZZ)   # 緑だけ
imgB = RGB.(ZZZ, ZZZ, BBB)   # 青だけ

plot(
    plot(imgR,  title="Grayscale Image (Red)",  axis=false, ticks=false),
    plot(imgG,  title="Grayscale Image (Green)",axis=false, ticks=false),
    plot(imgB,  title="Grayscale Image (Blue)", axis=false, ticks=false),
    plot(imgRGB,title="RGB Image",              axis=false, ticks=false),
...
begin
	RR = rand(80:255, 5, 10, 100) ./ 255
    GG = rand(80:255, 5, 10, 100) ./ 255
    BB = rand(80:255, 5, 10, 100) ./ 255

    anim2 = @animate for k in 1:size(RR, 3)
    imgRR   = RGB.(RR[:, :, k], 0, 0)
    imgGG   = RGB.(0, GG[:, :, k], 0)
    imgBB   = RGB.(0, 0, BB[:, :, k])
    imgRRGGBB = RGB.(RR[:, :, k], GG[:, :, k], BB[:, :, k])

    p1 = plot(imgRR,   title="Grayscale (Red)",   axis=false, ticks=false)
    p2 = plot(imgGG,   title="Grayscale (Green)", axis=false, ticks=false)
    p3 = plot(imgBB,   title="Grayscale (Blue)",  axis=false, ticks=false)
    p4 = plot(imgRRGGBB, title="RGB Image",         axis=false, ticks=false)
...
# Decomposition Filter 
	...
	for l=1:L1
		RXII[:,:,l]=dwt(RRR, wavelet(WT.haar), l);
		GXII[:,:,l]=dwt(GGG, wavelet(WT.haar), l);
		BXII[:,:,l]=dwt(BBB, wavelet(WT.haar), l);
	end
